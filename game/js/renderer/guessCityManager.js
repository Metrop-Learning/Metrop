...
export async function init(dataSet, nameList){
        ...
        e.addEventListener('click',()=>{
            if(util.checkDiff(document.getElementById('autoNaming').value,nameList[selector]) <= 1){
                if (trydone == 0){
                    ...
                    marker = new maplibregl.Marker({ color: "#32da49" })
                    ...
                    //marker
                    if (marker) marker.remove();
                    marker = new maplibregl.Marker({ color: "#d3732f" })
                      .setLngLat([dataSet[selector].lng,dataSet[selector].lat])
                      .addTo(map)
                  }
            }else {
                ...
                if(trydone < trymax){
                    ...
                    //marker
                    if (marker) marker.remove();
                    marker = new maplibregl.Marker({ color: "#d32f2f" })
                      .setLngLat([dataSet[selector].lng,dataSet[selector].lat])
                      .addTo(map)
                }
            }
    ...
    document.querySelectorAll('.continueBtn').forEach((e) => {
        e.addEventListener('click',()=>{
            ...
            listOfMarker[selector] = new maplibregl.Marker({ color: "#7c7c7c" })
              .setLngLat([dataSet[selector].lng,dataSet[selector].lat])
              .addTo(map);
            if (marker) marker.remove();
            ...
            if(selector >= dataSet.length){
                ...
                for(let i = 0; i < nameList.length; i++){
                    if(listOfMarker[i]){
                        listOfMarker[i].remove();
                    }
                }
                document.getElementById('selectNameActionBar').style.display= "none";
                document.getElementById('mapArea').style.display= "none";
                resultManager.show("guess",reccords,dataSet)
                return
            }
            listOfMarker[selector].remove();
            marker = new maplibregl.Marker({ color: "#3190d2" })
              .setLngLat([dataSet[selector].lng,dataSet[selector].lat])
              .addTo(map)
            map.flyTo({
