...
export async function init(dataSet, nameList){
    ...
    document.getElementById('mapShadowArea').style.display = "none";
    ...
    const promises = data.use.geojson(dataSet,"shadow").map(async (link) => {
    ...
    const applyMapData = () => {
        const bounds = geojson.addGeojsonListToExistingMap(results,mapShadow);

        document.getElementById('mapShadowArea').style.display = "flex";
        document.getElementById('selectNameActionBar').style.display = "flex";
        document.getElementById('loadingScreen').style.display = "none";

        geojson.changeState(0,"selected",mapShadow)
        for(let i = 1; i < nameList.length; i++){
            geojson.changeState(i,"hidden",mapShadow)
        }

        if (bounds) {
            setTimeout(()=>{
                geojson.adjustCamCountry(selector,results,mapShadow,0);
            },150)
        }
    };
    ...
    document.querySelectorAll('.continueBtn').forEach((e) => {
        e.addEventListener('click',()=>{
            ...
            //continue logic
            geojson.changeState(selector,"hidden",mapShadow)
            selector++;
            if(selector >= nameList.length){
                //Call the result here
                document.getElementById('selectNameActionBar').style.display= "none";
                document.getElementById('mapArea').style.display = "none";
                resultManager.show("shadowTerritory",reccords,dataSet)
                return
            }
            geojson.changeState(selector,"selected",mapShadow)
            geojson.adjustCamCountry(selector,results,mapShadow,0);
