...
document.getElementById("warningSwitchLink").addEventListener("click", () => {
  const params = new URLSearchParams({
      json: jsonName,
      learningID: -1,
      return: returned
  ...
  normalQuit = true
  window.location.replace("../guess/index.html?" + params);
});
...
document.getElementById("continueMenu").addEventListener("click", () => {
  if(miss.length / (fail + miss.length + good.length) < 0.1 && fail == 0 && posi.length == learningID){
