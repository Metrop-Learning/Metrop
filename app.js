...
if(localStorage.getItem("SETTINGS_CUSTOMISATION_IMG_SHOW") == "true"){
    document.documentElement.style.setProperty('--img-show', 'block');
} else{
    document.documentElement.style.setProperty('--img-show', 'none');
}
...
async function cityListSetUp(nameJson,type){
    ...
    if ('cardInfo' in obj) {
            ...
            if ('setInfo' in obj.cardInfo) {
                ...
                dataToReturn["flag"] = null
                dataToReturn["countryName"] = null
                dataToReturn["continent"] = "WD"
                        
                if (continent && dataBorder[continent]) {
                  deep = dataBorder[continent];
                  dataToReturn["continent"] = deep.namefr
                  if ('flag' in deep){
                    dataToReturn["flag"] = deep.flag
                  }
                }
    ...
    if(flagImg){
        ...
        h3.innerHTML = "<img class='flagOnCard' src='" + dataBorder.WD.flag + "'><br>" + h3.innerHTML
    }
    h3.className = 'top'
...
async function TodayQuiz(){
    ...
    if (!jsonName_quiz || date_quiz < today){
        ...
        let choosen;
        if(Math.random() > (lQv / (lQc + lQv))){
            choosen = listCountryQuizInfo[Math.floor(Math.random() * listCountryQuizInfo.length)];
        }else{
            choosen = listCityQuizInfo[Math.floor(Math.random() * listCityQuizInfo.length)];
        }
        localStorage.setItem("TODAY_QUIZ_DATE",today)
        obj = JSON.stringify(choosen);
        localStorage.setItem("TODAY_QUIZ",obj)
        jsonName_quiz = localStorage.getItem("TODAY_QUIZ")
    }
    obj = JSON.parse(jsonName_quiz);
    document.getElementById('todayQuizTittleCountry').innerText = obj.countryName ?? obj.countryID ?? obj.continent ?? "Monde"
    document.getElementById('todayQuizDes').innerText = obj.name
...
document.getElementById('showImg').addEventListener("change", () => {
    if(document.getElementById('showImg').checked == true){
        document.documentElement.style.setProperty('--img-show', 'block');
        localStorage.setItem("SETTINGS_CUSTOMISATION_IMG_SHOW","true")
    } else{
        document.documentElement.style.setProperty('--img-show', 'none');
        localStorage.setItem("SETTINGS_CUSTOMISATION_IMG_SHOW","false")
    }
});

async function explore(jsonName,type){
  document.getElementById('popupMainInfoQuiz').style.display = 'block'
  const obj = await loadJSON(jsonName,type);
  if (obj == undefined) return
  ...
  if(type == "city"){
    document.getElementById('cardInfoPopUp_info_lenght_txt').innerText = obj.listCity.length
  }
  if(type == "country"){
    document.getElementById('cardInfoPopUp_info_lenght_txt').innerText = obj.listCountry.length
  }
  if(type == "flag"){
    document.getElementById('cardInfoPopUp_info_lenght_txt').innerText = obj.listFlag.length
  }
  document.getElementById('cardInfoPopUp_imgFlag').src = getAElement(obj.cardInfo.setInfo)?.flag ?? dataBorder.WD.flag
  ...
  if(obj.type.includes("place")){
    let btnPlace1 = '<button class="btn" onclick="placeOnMap('+"'"+jsonName+"'"+')"'
