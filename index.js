...
try{
    ...
    console.error("Imposible to join supabase");
}
...
for(let i = 0; i < Math.min(6,listOfShowcase.length);i++){
    ...
    for(let j = 0; j < listOfShowcase[i][1].list.length; j++){
        ...
        cards.innerHTML +=
            `<div class='card' onclick="explore(${listOfShowcase[i][1].list[j][0]},${listOfShowcase[i][1].list[j][1]})"><img class='flagCard' src='${flagSrc}'><p class="titleCard">${info.cardInfo.Title}</p><div class="exploreCard">${exploreSVG}<p>Explore</p></div></div>`
    }
